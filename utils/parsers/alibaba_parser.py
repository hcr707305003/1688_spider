...
class AlibabaParser(BaseParser):
    ...
    def _get_context_data(self) -> Optional[Dict]:
        """读取新版详情页注入到 window.context 的 JSON 数据。"""
        if self._context_loaded:
            return self._context_data

        self._context_loaded = True
        marker = '})(window.contextPath,'
        marker_index = self.html_content.find(marker)
        if marker_index == -1:
            return None

        payload = self.html_content[marker_index + len(marker):].lstrip()
        payload = re.sub(
            r'([{,])(\d+):',
        ...
        try:
            self._context_data, _ = json.JSONDecoder().raw_decode(payload)
    ...
    def get_detail_images(self) -> List[str]:
        ...
        if not detail_images:
            context = self._get_context_data() or {}
            detail_url = (
                context.get('result', {})
                .get('data', {})
                .get('description', {})
            ...
            if detail_url:
                try:
                    ...
                    prefix = 'var offer_details='
                    if payload.startswith(prefix):
                        payload = payload[len(prefix):].rstrip(';')
                    detail_data = json.loads(payload)
                    detail_soup = BeautifulSoup(detail_data.get('content', ''), 'html.parser')
    ...
    def get_attributes(self) -> List[Tuple[str, str]]:
        ...
        # 4. 新版详情页会把完整属性放在 window.context 中。
        context = self._get_context_data() or {}
        context_attributes = (
            context.get('result', {})
            .get('global', {})
        ...
        if isinstance(context_attributes, list):
            for item in context_attributes:
    ...
    def get_price(self) -> Optional[Dict]:
        ...
        price_info = extractor.extract_all_prices()
        ...
        price_ranges = price_model.get('currentPricesWithOnePiece', [])

        if not price_info.get('ladder_prices') and isinstance(price_ranges, list):
            ladder_prices = []
            seen_amounts = set()
            for item in price_ranges:
                if not isinstance(item, dict):
                    continue
                if '$ref' in item and current_prices:
                    item = current_prices[0]
                try:
                    min_amount = int(item['beginAmount'])
                    price = float(item['price'])
                except (KeyError, TypeError, ValueError):
                    continue
                if min_amount not in seen_amounts:
                    seen_amounts.add(min_amount)
                    ladder_prices.append({
                        'min_amount': min_amount,
                        'price': price
                    })
            price_info['ladder_prices'] = ladder_prices

        if price_info.get('main_price') and current_prices:
            price_info['main_price'].setdefault(
                'min_amount',
                int(current_prices[0].get('beginAmount', trade_model.get('beginAmount', 1)))
            )
    ...
    def get_sku_matrix(self) -> List[Dict]:
        ...
        if isinstance(sku_data, list):
            price_info = self.get_price() or {}
            main_price = (price_info.get('main_price') or {}).get('price', 0)
            for sku_info in sku_data:
                sku_list.append({
                    'sku_name': unescape(sku_info.get('specAttrs', '')),
                    'sku_id': str(sku_info.get('skuId', '')),
                    'price': float(sku_info.get('price', main_price) or 0),
                    'original_price': float(sku_info.get('discountPrice', 0) or 0),
                    'stock': int(sku_info.get('canBookCount', 0) or 0)
                })
    ...
    def get_all_info(self) -> Dict:
        """获取商品信息，并补充新版页面中的 SKU 明细。"""
        info = super().get_all_info()
        info['sku_matrix'] = self.get_sku_matrix()
        return info
    ...
    def get_shop_info(self) -> Optional[Dict]:
        ...
        if seller:
            shop_info.setdefault('shop_name', seller.get('companyName', ''))
            shop_info.setdefault(
                'shop_url',
                seller.get('winportUrl')
                or seller.get('sellerWinportUrlMap', {}).get('defaultUrl', '')
            )
            shop_info.setdefault('shop_id', str(seller.get('userId', '')))
            shop_info['member_id'] = seller.get('memberId', '')
            shop_info['login_id'] = seller.get('loginId', '')
